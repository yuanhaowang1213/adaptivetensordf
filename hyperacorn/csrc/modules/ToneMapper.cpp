...
{
    auto result = torch::zeros({1}, torch::TensorOptions().device(device));
    ...
    target.slice(3, NumParameters() - 1, NumParameters()).fill_(range);
...
torch::Tensor PhotometricCalibrationImpl::forward(torch::Tensor ray_integral, torch::Tensor uv, torch::Tensor image_id)
{
    ...
    {
        // printf("perform exposure estimation\n");
...
void PhotometricCalibrationImpl::ApplyConstraints()
{
    ...
    // exposure_bias.add_(exposure_bias.min().clamp_max_(0) * 0.1);
    //     sensor_bias.add_(sensor_bias.min().clamp_max_(0));

    // Make sure the bias is always positive!
    if(params.use_expose_constraint)
    {
