...
// template <typename scalar_t>
void grid_encode_forward(const at::Tensor inputs, const at::Tensor embeddings, const at::Tensor offsets, at::Tensor outputs, const uint32_t B, const uint32_t D, const uint32_t C, const uint32_t L, const float S, const uint32_t H, torch::Tensor dy_dx, const uint32_t gridtype, const bool align_corners) {
    CHECK_CUDA(inputs);
    ...
    // printf("test hash code grid encoder forward 00 \n");
    ...
    embeddings.scalar_type(), "grid_encode_forward", ([&] {
        grid_encode_forward_cuda<scalar_t>(inputs.data_ptr<float>(), embeddings.data_ptr<scalar_t>(), offsets.data_ptr<int>(), outputs.data_ptr<scalar_t>(), B, D, C, L, S, H, dy_dx.defined() ? dy_dx.data_ptr<scalar_t>() : nullptr, gridtype, align_corners);
    }));
    // AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    // embeddings.scalar_type(), "grid_encode_forward", ([&] {
    //     grid_encode_forward_cuda<scalar_t>(inputs.data_ptr<float>(), embeddings.data_ptr<float>(), offsets.data_ptr<int>(), outputs.data_ptr<float>(), B, D, C, L, S, H, dy_dx.has_value() ? dy_dx.value().data_ptr<float>() : nullptr, gridtype, align_corners);
    // }));
}
// template <typename scalar_t>
void grid_encode_backward(const at::Tensor grad, const at::Tensor inputs, const at::Tensor embeddings, const at::Tensor offsets, at::Tensor grad_embeddings, const uint32_t B, const uint32_t D, const uint32_t C, const uint32_t L, const float S, const uint32_t H, const torch::Tensor dy_dx, torch::Tensor grad_inputs, const uint32_t gridtype, const bool align_corners) {
    CHECK_CUDA(grad);
    CHECK_CUDA(inputs);
    CHECK_CUDA(embeddings);
    CHECK_CUDA(offsets);
    ...
    grad.scalar_type(), "grid_encode_backward", ([&] {
        grid_encode_backward_cuda<scalar_t>(grad.data_ptr<scalar_t>(), inputs.data_ptr<float>(), embeddings.data_ptr<scalar_t>(), offsets.data_ptr<int>(), grad_embeddings.data_ptr<scalar_t>(), B, D, C, L, S, H, dy_dx.defined() ? dy_dx.data_ptr<scalar_t>() : nullptr, grad_inputs.defined() ? grad_inputs.data_ptr<scalar_t>() : nullptr, gridtype, align_corners);
    }));
