...
{
   public:
    HyperTreeBaseImpl(int d_, int max_depth_);
    void settree(bool use_quad, float y_min, float y_max);
    // HyperTreeBaseImpl(int d, int max_depth, bool use_quad, float y_min, float y_max);

    // HyperTreeBaseImpl(std::vector<int> depth, std::vector<float> min_max);
    ...
    //
    SampleList NodeNeighborSamples(Eigen::Vector<int, -1> size, double epsilon, vec3 roi_min, vec3 roi_max, bool use_roi, bool use_quad);

    SampleList NodeNeighborSamplesPlane2D(Eigen::Vector<int, -1> size, double epsilon,  int plane_index, vec3 roi_min, vec3 roi_max, bool use_roi, bool use_quad);
    ...
    SampleList CreateSamplesForRays_area(const RayList& rays, int max_samples_per_node, bool interval_jitter, vec3 tree_roi_min, vec3 tree_roi_max);

    torch::Tensor CreateSamplesRandomly(torch::Tensor node_id, int num_of_samples_per_edge);

    torch::Tensor ComputeBoundWeight(torch::Tensor node_id, torch::Tensor in_roi_node, bool out_selected);

    std::tuple<torch::Tensor, torch::Tensor> ComputeRatio(torch::Tensor weight_in, torch::Tensor weight_all);
    ...
    {
        auto res = torch::index_select(node_active_prefix_sum, 0, node_id.reshape({-1}).contiguous());
        res = res.reshape(node_id.sizes()).contiguous();
