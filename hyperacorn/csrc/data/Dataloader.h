...
class RandomRaybasedSampleDataset : public torch::data::BatchDataset<RandomRaybasedSampleDataset, SampleData>
{
   public:
    RandomRaybasedSampleDataset(std::vector<int> indices, std::shared_ptr<SceneBase> scene,
                                std::shared_ptr<CombinedParams> params, int rays_per_image)
        : indices(indices), scene(scene), params(params), rays_per_image(rays_per_image)
    {
        CHECK_GT(rays_per_image, 0);
    }
    virtual torch::optional<size_t> size() const override { return indices.size() * rays_per_image; }
